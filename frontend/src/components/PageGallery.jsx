...
export default function PageGallery({ images: propImages, title: propTitle }) {
  ...
  useEffect(() => {
    const fetchGlobalGallery = async () => {
      try {
        ...
        const apiBase = envApiBase || "http://localhost:5000/api";
        const portal = "landing1";
        const fetchUrl = `${apiBase}/${portal}/page-galleries/by-url?url=${location.pathname}`;
        
        console.log("PageGallery fetching:", fetchUrl);
        
        const res = await fetch(fetchUrl);
        if (res.ok) {
          const data = await res.json();
          console.log("PageGallery got data:", data);
          ...
          console.error("PageGallery response not OK:", res.status);
        }
      } catch (err) {
  ...
  console.log("PageGallery actualImages:", actualImages);
